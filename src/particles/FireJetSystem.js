...
// Standalone, pensato per essere annidato dentro il FireBreathCone.
...
export class FireJetSystem {
  constructor(opts = {}) {
    ...
    this.particleCount = opts.particleCount ?? 900;
  ...
  _createParticles() {
    ...
    // Bounding sphere molto ampia per evitare culling
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main(){
          ...
          // point size: grande vicino a metà cono, poi decresce
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          // molto caldo vicino all'inizio del jet
          ...
          // core più bianco al centro del punto
  ...
  _reset(i, pos, vel, life, size, seed){
    ...
    // velocità: prevale Z ma più morbida (meno “treno in corsa”)
    const speed = 2.0 + Math.random() * 2.8;
    const sXY   = (Math.random() - 0.5) * 0.5;
    vel[i3]     = sXY * 0.45;
    vel[i3 + 1] = (Math.random() - 0.5) * 0.25;  // ~[-0.125..+0.125]
    vel[i3 + 2] = speed;

    life[i] = Math.random();              // [0..1)
    size[i] = 0.65 + Math.random()*0.55;  // scala punto
    seed[i] = Math.random();
  }
  ...
  attachTo(parent, localOffset=new THREE.Vector3()){
    ...
    this.group.rotation.set(0,0,0); // già orientato su +Z
  }
