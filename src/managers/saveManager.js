...
function snapshotInventory(inv) {
  if (!inv) return null;
  const itemIds = Array.isArray(inv.items) ? inv.items.map(it => it?.id).filter(Boolean) : [];
...
function applyInventory(gm, invSnap) {
  ...
  const items = (invSnap.itemIds || []).map(id => getItemById(id)).filter(Boolean);
  inv.items = items;
  inv.equipment = { weapon: null, shield: null, helmet: null };

  const equipId = invSnap.equipment || {};
  const equipIfPresent = (slot, id) => { if (id) inv.equip(id, slot); };
