...
export class BushSpawner {
  constructor({
    scene,
    getTerrainHeightAt,
    textures = {
      bark: { map: '/textures/bush/Bark.jpeg', normalMap: '/textures/bush/Bark_Normal.jpeg' },
      leaf: { map: '/textures/bush/Leaf.jpeg', normalMap: '/textures/bush/Leaf_Normal.jpeg', alphaMap: '/textures/bush/Leaf_Opacity.jpeg' }
    },
    yoffset = 0,
    baseScale = 0.1,
    randomScaleRange = [0.5, 1.2],
    alphaTest = 0.5,
    anisotropy = 8,
    castShadow = true,
    receiveShadow = true,
    defaultBarkMaterialIndex = 2,
    ...
    if (!scene) throw new Error('BushSpawner: scene required');
    if (!getTerrainHeightAt) throw new Error('BushSpawner: getTerrainHeightAt required');
  ...
  async _getMergedGeometries(modelPath, barkIndex) {
    ...
    const bakeRot = new THREE.Matrix4().makeRotationX(-Math.PI / 2);
  ...
  async _createBatch(matrices, hasBark, hasLeaf, barkGeom, leafGeom) {
    ...
    const createLODLevel = (matrices, targetReduction) => {
      if (matrices.length <= 4) return matrices;
  ...
  optimizeBatches() {
    const smallBatches = this._batches.filter(b => (b.matrices.L1?.length || 0) < this.maxInstancesPerBatch / 4);
