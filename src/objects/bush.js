...
export class BushSpawner {
  constructor({
    ...
    if (!scene) throw new Error('BushSpawner: "scene" è richiesto.');
    if (!getTerrainHeightAt) throw new Error('BushSpawner: "getTerrainHeightAt" è richiesto.');
  ...
  setDebugLOD(enabled) {
    ...
    console.log(`BushSpawner debug ${enabled ? 'enabled' : 'disabled'}`);
  ...
  logPerformanceStats() {
    ...
    console.group('BushSpawner Performance Stats');
