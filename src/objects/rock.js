...
export class SmallRockSpawner {
  constructor({
    scene,
    getTerrainHeightAt,
    textures = {
      map: '/textures/smallrock/smallrocksColor.png',
    ...
    this.stoneMaterial = new THREE.MeshStandardMaterial({
      ...
      normalMap: rockTex.normalMap || null,
      roughness: rockTex.roughnessMap ? 1.0 : 0.9,
      roughnessMap: rockTex.roughnessMap || null,
      metalness: 0.0,
      fog: true,
      dithering: true
    });
    if (this.stoneMaterial.map) this.stoneMaterial.map.colorSpace = THREE.SRGBColorSpace;
  ...
  async _getMergedGeometries(modelPath) {
    const key = `${modelPath}__rock`;
    if (this._modelCache.has(key)) return this._modelCache.get(key);

    const fbx = await this.loader.loadAsync(modelPath);
    const bakeRot = new THREE.Matrix4().makeRotationX(this.rotationoffset || 0);
    ...
    const rockGeom = parts.length ? BufferGeometryUtils.mergeGeometries(parts, false) : new THREE.BufferGeometry();
    if (rockGeom.attributes.position) {
      rockGeom.computeBoundingSphere();
      rockGeom.computeBoundingBox();
    }

    this._modelCache.set(key, rockGeom);
    return rockGeom;
  }
  ...
  async spawn(modelPath, count, area, opts = {}) {
    ...
    for (const mats of cellMap.values()) {
      for (let off = 0; off < mats.length; off += this.maxInstancesPerBatch) {
        ...
        await this._createBatch(chunk, rockGeom);
      }
    }
  }

  async _createBatch(matrices, rockGeom) {
    const rockIMesh = new THREE.InstancedMesh(rockGeom, this.stoneMaterial, matrices.length);
  ...
  updateLOD(camera) {
    ...
    this._stats.culledInstances = Math.max(0, (this._batches.length * this.maxInstancesPerBatch) - totalVisible);
  }
  ...
  _switchLOD(batch, newLod) {
    ...
    if (isVisible) {
      ...
      this._updateBatchInstances(batch.rockIMesh, matrices);
      ...
      if (batch.rockIMesh) {
        batch.rockIMesh.castShadow = enableShadowCasting && this._castShadow;
        batch.rockIMesh.receiveShadow = enableShadowReceiving && this._receiveShadow;
      }
    } else {
  ...
  _setBatchVisibility(batch, visible) {
    ...
    if (batch.rockIMesh) {
      batch.rockIMesh.visible = visible;
      if (visible) batch.rockIMesh.instanceMatrix.needsUpdate = true;
    }
  }
